...
/**
 * All Redback structures inherit from this base structure.
 */
...
Structure.prototype.getKey = function (key) {
...
Structure.prototype.getKeys = function (structures) {
    ...
    for (var i = 0, l = structures.length; i < l; i++) {
        if (Array.isArray(structures[i])) {
            structures[i].forEach(function (structure) {
                keys.push(self.getKey(structure));
            });
        } else {
            keys.push(this.getKey(structures[i]));
        }
    }
    keys.push(callback);
    return keys;
